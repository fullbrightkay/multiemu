...
use egui::FullOutput;
use egui::TextureId;
use nalgebra::{DMatrix, DMatrixViewMut, Matrix2x3, Point2, Vector2, Vector3, Vector4};
...
fn barycentric_coordinates(point: Point2<f32>, v: Matrix2x3<f32>) -> Vector3<f32> {
    ...
    let area2 = triangle_area(Matrix2x3::from_columns(&[
        v.column(0).into(),
        point.coords,
        v.column(2).into(),
    ]));
    let area3 = triangle_area(Matrix2x3::from_columns(&[
        v.column(0).into(),
        v.column(1).into(),
        point.coords,
    ]));
